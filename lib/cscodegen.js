...
(function() {
  ...
  (function(exports) {
    ...
    return exports.generate = generate = function(ast, options) {
      var a, absNum, arg, argList, args, assignee, block, body, ctor, elseBlock, expr, fn, i, indexingExpr, isBlockMultiline, isElseBlockMultiline, left, needsParens, op, p, paramList, parameters, parent, parentClassName, prec, right, s, sep, src, usedAsExpression, _block, _elseBlock, _kw, _ref, _ref1;
      if (options == null) {
        options = {};
      }
      needsParens = false;
      ...
      src = (function() {
        ...
        switch (ast.className) {
          ...
          case 'Conditional':
            options.ancestors.unshift(ast);
            options.precedence = 0;
            isElseBlockMultiline = (ast.elseBlock != null) && ast.elseBlock.className === 'Block' && ast.elseBlock.statements.length > 1;
            isBlockMultiline = isElseBlockMultiline || (ast.block != null) && ast.block.className === 'Block' && ast.block.statements.length > 1;
            block = ast.block != null ? (_block = generate(ast.block, options), isBlockMultiline ? "\n" + (indent(_block)) : " then " + _block) : " then " + (generate((new Undefined).g(), options));
            elseBlock = ast.elseBlock != null ? (_kw = "" + (isBlockMultiline ? '\n' : ' ') + "else", _elseBlock = generate(ast.elseBlock, options), isElseBlockMultiline ? "" + _kw + "\n" + (indent(_elseBlock)) : "" + _kw + " " + _elseBlock) : "";
            return "if " + (generate(ast.condition, options)) + block + elseBlock;
          case 'Identifier':
            return ast.data;
          case 'Null':
            return 'null';
          ...
          case 'CompoundAssignOp':
            ...
            assignee = generate(ast.assignee, options);
            expr = generate(ast.expr, options);
            return "" + assignee + " " + operators[ast.op.prototype.className] + "= " + expr;
          case 'SeqOp':
            prec = precedence[ast.className];
            needsParens = prec < options.precedence;
            options.precedence = prec;
